...
unsigned long message_size_packetsreceived(struct msg_packetsreceived * h) {
  return  strlen_ethanol(h->p_version) + sizeof(h->m_type) + sizeof(h->m_size) + sizeof(h->m_id) +
          ...
          sizeof(h->packet_received);
}

void encode_msg_packetsreceived(struct msg_packetsreceived * h, char ** buf, int * buf_len) {
	*buf_len = message_size_packetsreceived(h);
	*buf = malloc(*buf_len);
	char * aux = *buf;
	h->m_size = *buf_len;
  ...
  encode_int(&aux, h->sta_port);
	encode_2long(&aux, h->packet_received);
}

void decode_msg_packetsreceived(char * buf, int buf_len, struct msg_packetsreceived ** h){
   *h = malloc(sizeof(struct msg_packetsreceived));
	char * aux = buf;
  ...
  decode_int(&aux, &(*h)->sta_port);
	decode_2long(&aux, &(*h)->packet_received);
}


void process_msg_packetsreceived(char ** input, int input_len, char ** output, int * output_len) {

  struct msg_packetsreceived * h;
  decode_msg_packetsreceived(*input, input_len, &h);

  /************************************ FUNCAO LOCAL ***********************************/
  if (h->sta_ip != NULL) {
    // should call the station
    int id = h->m_id;
    struct msg_packetsreceived * h1 = send_msg_get_packetsreceived(h->sta_ip, h->sta_port, &id, h->intf_name, NULL, 0);
    if (h1 != NULL) {
      h->packet_received = h1->packet_received;
    ...
    // @ STATION
        ...
        if ((stats = get_interface_stats(h->intf_name)) != NULL) {
          h->packet_received = stats->rx_packets;
          free(stats);
        } else {
          h->packet_received = 0;
        }
  ...
  encode_msg_packetsreceived(h, output, output_len);

  /************************************ FIM FUNCAO LOCAL *******************************/

  #ifdef DEBUG
    print_packetsreceived(h);
  #endif

  // liberar h
  free_msg_packetsreceived(h);
}
...
struct msg_packetsreceived * send_msg_get_packetsreceived(char * hostname, int portnum, int * id, char * intf_name, char * sta_ip, int sta_port) {
  ...
  struct ssl_connection h_ssl;
	struct msg_packetsreceived * h1 = NULL;
  ...
  if (err == 0 && NULL != h_ssl.ssl) {
		...
		struct msg_packetsreceived h;
		h.m_type = (int) MSG_GET_PACKETSRECEIVED;
		h.m_id = (*id)++;
		h.p_version = NULL;
    ...
    h.sta_port = sta_port;
    h.packet_received = 0; // este valor não importa pois será retornado por process_

		h.m_size = message_size_packetsreceived(&h);
  	...
  	encode_msg_packetsreceived(&h, &buffer, &bytes);
    ...
    if (return_message_type((char *)&buf, bytes) == MSG_GET_PACKETSRECEIVED)
      decode_msg_packetsreceived((char *)&buf, bytes, &h1);
