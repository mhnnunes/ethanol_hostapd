...
static int print_phy_handler_bitrates(struct nl_msg *msg, void *arg)
{
    struct nlattr *tb_msg[NL80211_ATTR_MAX + 1];
    struct genlmsghdr *gnlh = nlmsg_data(nlmsg_hdr(msg));
    ...
    static char * wiphy = NULL;
    if (print_name && tb_msg[NL80211_ATTR_WIPHY_NAME]) {
    	char * w = nla_get_string(tb_msg[NL80211_ATTR_WIPHY_NAME]);
    	if (wiphy) free(wiphy);
    	wiphy = malloc(sizeof(char) * (strlen(w)+1));
    	strcpy(wiphy, w);
    }
    ...
    if (tb_msg[NL80211_ATTR_WIPHY_BANDS]) {
        nla_for_each_nested(nl_band, tb_msg[NL80211_ATTR_WIPHY_BANDS], rem_band) {
            if (last_band != nl_band->nla_type) {
            	band = nl_band->nla_type + 1;
            }
            last_band = nl_band->nla_type;
            if (band != 0) {
				printf("Wiphy %s\tBand %d :", wiphy, band);
	            ...
	            if (tb_band[NL80211_BAND_ATTR_RATES]) {
		            ...
		            nla_for_each_nested(nl_rate, tb_band[NL80211_BAND_ATTR_RATES], rem_rate) {
		                ...
		                if (!tb_rate[NL80211_BITRATE_ATTR_RATE])
		                    continue;
		                printf("%2.1f,", 0.1 * nla_get_u32(tb_rate[NL80211_BITRATE_ATTR_RATE]));
		                //int open = 0;
		                if (tb_rate[NL80211_BITRATE_ATTR_2GHZ_SHORTPREAMBLE])
		                    printf("S,");
		                else
...
// new command based on "iw phy phy0 info"
// lists only bitrates
static int handle_info_bitrates(struct nl80211_state *state,
		       struct nl_msg *msg,
		       int argc, char **argv,
		       enum id_input id)
{
	...
	register_handler(print_phy_handler_bitrates, NULL);

	return 0;
}
__COMMAND(NULL, bitrates, "bitrates", NULL, NL80211_CMD_GET_WIPHY, 0, 0, CIB_PHY, handle_info_bitrates,
	 "Show tx bitrates for the specified wireless device.", NULL);
#endif
