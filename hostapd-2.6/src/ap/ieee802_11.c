...
#include "messaging/msg_association_process.h"
#endif
...
static void handle_auth(struct hostapd_data *hapd,
            const struct ieee80211_mgmt *mgmt, size_t len, bool dont_auth)
#else
static void handle_auth(struct hostapd_data *hapd,
            const struct ieee80211_mgmt *mgmt, size_t len)
#endif
...
#ifdef ETHANOL
    if (dont_auth) {
        wpa_printf(MSG_INFO, "STA=" MACSTR " refused by controller",  MAC2STR(mgmt->sa));
        ...
        struct hostapd_frame_info *fi)
{
  ...
  if (len < IEEE80211_HDRLEN + sizeof(mgmt->u.beacon)) {
      #ifdef ETHANOL
        /*wpa_printf(MSG_INFO, "handle_beacon - too short payload (len=%lu) STA " MACSTR,
  ...
  /** ethanol's beacon handler is inside this function */
        ...
        struct hostapd_frame_info *fi)
{
  ...
  default:
    ...
    static int m_id = 0;
    char mac_ap[20];
    sprintf((char *)&mac_ap, MACSTR, MAC2STR(hapd->own_addr));
    char mac_sta[20];
    sprintf((char *)&mac_sta, MACSTR, MAC2STR(mgmt->sa));
    ...
    struct msg_association_process * m = NULL;
    bool denied_by_controller = false;
    switch (stype) {
    case WLAN_FC_STYPE_AUTH:
        wpa_printf(MSG_DEBUG, "mgmt::auth");
        if (config.enabled_msg_authorization) {
          m = send_msg_association_process(
              config.server_addr, config.remote_server_port, &m_id,
              (char *)&mac_ap, (char *)&mac_sta,
              AUTHORIZATION);
          if (m) {
              denied_by_controller = !m->allowed;
              free_msg_association_process(m);
          }
        }
        handle_auth(hapd, mgmt, len, denied_by_controller);
        ret = 1;
        break;
    case WLAN_FC_STYPE_ASSOC_REQ:
        wpa_printf(MSG_DEBUG, "mgmt::assoc_req");
        if (config.enabled_msg_association) {
          m = send_msg_association_process(
              config.server_addr, config.remote_server_port, &m_id,
              (char *)&mac_ap, (char *)&mac_sta,
              ASSOCIATION);
        ...
        handle_assoc(hapd, mgmt, len, 0, denied_by_controller); // 0 = association
        ret = 1;
        break;
    case WLAN_FC_STYPE_REASSOC_REQ:
        wpa_printf(MSG_DEBUG, "mgmt::reassoc_req");
        if (config.enabled_msg_reassociation) {
          m = send_msg_association_process(
              config.server_addr, config.remote_server_port, &m_id,
              (char *)&mac_ap, (char *)&mac_sta,
              REASSOCIATION);
        ...
        handle_assoc(hapd, mgmt, len, 1, denied_by_controller);
        ret = 1;
        break;
    case WLAN_FC_STYPE_DISASSOC:
        wpa_printf(MSG_DEBUG, "mgmt::disassoc");
        if (config.enabled_msg_reassociation) {
          m = send_msg_association_process(
              ...
              (char *)&mac_ap, (char *)&mac_sta,
              DISASSOCIATION);
          if (m) free_msg_association_process(m);
        }
        /** there is nothing you can do if the sta wants to disassociate
            just log it
         */
        handle_disassoc(hapd, mgmt, len);
        ret = 1;
