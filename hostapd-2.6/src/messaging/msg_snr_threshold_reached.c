...
void process_msg_snr_threshold_reached(char ** input, int input_len, char ** output, int * output_len) {
  ...
  if (h->sta_ip == NULL) {
            ...
            if (sta) {
                ...
                for(i = 0; i < l_aps->num_aps; i++) {
                    ...
                    if (strcmp(l_aps->aps[i].mac_ap, h->mac_ap)!=0 && // ap is not the current ap
                        (ap=find_ap(l_aps->aps[i].mac_ap)))           // ap is in ethanol's list
                    {
    ...
    struct msg_snr_threshold_reached * h1 = send_msg_snr_threshold_reached(h->sta_ip, h->sta_port, &h->m_id, NULL, 0, h->intf_name, h->mac_ap, h->snr);
    if (h1) {
