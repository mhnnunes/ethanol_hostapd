...
static int print_sta_handler(struct nl_msg *msg, void *arg)
{
   ...
   */
    ...
    long long rx_packets = -1;
    long long rx_duration = -1;
    long long tx_packets = -1;
    long long tx_retries = -1;
    long long tx_failed = -1;
    long long beacon_loss = -1;
    long long beacon_rx = -1;
    long long rx_drop_misc = -1;
    int signal = -1;
    int signal_avg = -1;
    int beacon_signal_avg = -1;
    long long time_offset = -1;
    long long connected_time = -1;

    if (sinfo[NL80211_STA_INFO_INACTIVE_TIME]) inactive_time = nla_get_u32(sinfo[NL80211_STA_INFO_INACTIVE_TIME]);

    if (sinfo[NL80211_STA_INFO_RX_BYTES64]) {
      rx_bytes = (long long)nla_get_u64(sinfo[NL80211_STA_INFO_RX_BYTES64]);
    } else if (sinfo[NL80211_STA_INFO_RX_BYTES])
      rx_bytes = nla_get_u32(sinfo[NL80211_STA_INFO_RX_BYTES]);

    if (sinfo[NL80211_STA_INFO_RX_PACKETS]) rx_packets = nla_get_u32(sinfo[NL80211_STA_INFO_RX_PACKETS]);

    if (sinfo[NL80211_STA_INFO_TX_BYTES64]) {
      tx_bytes = (long long)nla_get_u64(sinfo[NL80211_STA_INFO_TX_BYTES64]);
    } else if (sinfo[NL80211_STA_INFO_TX_BYTES])
      tx_bytes = nla_get_u32(sinfo[NL80211_STA_INFO_TX_BYTES]);

    if (sinfo[NL80211_STA_INFO_TX_PACKETS]) tx_packets = nla_get_u32(sinfo[NL80211_STA_INFO_TX_PACKETS]);

    if (sinfo[NL80211_STA_INFO_TX_RETRIES]) tx_retries = nla_get_u32(sinfo[NL80211_STA_INFO_TX_RETRIES]);

    if (sinfo[NL80211_STA_INFO_TX_FAILED]) tx_failed = nla_get_u32(sinfo[NL80211_STA_INFO_TX_FAILED]);

    if (sinfo[NL80211_STA_INFO_BEACON_LOSS]) beacon_loss = nla_get_u32(sinfo[NL80211_STA_INFO_BEACON_LOSS]);

    if (sinfo[NL80211_STA_INFO_BEACON_RX]) beacon_rx = (long long)nla_get_u64(sinfo[NL80211_STA_INFO_BEACON_RX]);

    if (sinfo[NL80211_STA_INFO_RX_DROP_MISC]) rx_drop_misc = (long long)nla_get_u64(sinfo[NL80211_STA_INFO_RX_DROP_MISC]);

    if (sinfo[NL80211_STA_INFO_SIGNAL]) signal = (int8_t)nla_get_u8(sinfo[NL80211_STA_INFO_SIGNAL]);

    if (sinfo[NL80211_STA_INFO_SIGNAL_AVG]) signal_avg = (int8_t)nla_get_u8(sinfo[NL80211_STA_INFO_SIGNAL_AVG]);

    if (sinfo[NL80211_STA_INFO_BEACON_SIGNAL_AVG]) beacon_signal_avg = nla_get_u8(sinfo[NL80211_STA_INFO_BEACON_SIGNAL_AVG]);

    if (sinfo[NL80211_STA_INFO_T_OFFSET]) time_offset = (long long)nla_get_u64(sinfo[NL80211_STA_INFO_T_OFFSET]);

    if (sinfo[NL80211_STA_INFO_RX_DURATION]) rx_duration = (long long)nla_get_u64(sinfo[NL80211_STA_INFO_RX_DURATION]);

    if (sinfo[NL80211_STA_INFO_CONNECTED_TIME]) connected_time = nla_get_u32(sinfo[NL80211_STA_INFO_CONNECTED_TIME]);

    float tx_bitrate = -1;
    if (sinfo[NL80211_STA_INFO_TX_BITRATE]) {
      char buf[100];
      parse_bitrate(sinfo[NL80211_STA_INFO_TX_BITRATE], buf, sizeof(buf));
      sscanf(buf, "%f", &tx_bitrate);
      //printf("tx_bitrate buf: %s - convert %f\n", buf, tx_bitrate);
    }

    printf("%s %s %ld %lli %lli %lli %lli %lli %lli %lli %lli %lli %lli %d %d %d %lli %lli %2.1f\n",
            mac_addr, dev,
            inactive_time,
            rx_bytes, rx_packets, rx_duration,
            tx_bytes, tx_packets, tx_retries, tx_failed,
            beacon_loss, beacon_rx, rx_drop_misc,
            signal, signal_avg, beacon_signal_avg,
            time_offset, connected_time,
            tx_bitrate);
