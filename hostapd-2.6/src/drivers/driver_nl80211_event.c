          ...
          )
{
  ...
  if (len > 24 + sizeof(mgmt->u.auth)) {
    ...
    int ssi_signal = 0;
    int rx_freq = 0;
    if (sig) ssi_signal = (s32) nla_get_u32(sig);
    if (freq) rx_freq = nla_get_u32(freq);
    event.rx_mgmt.ssi_signal = ssi_signal;
    event.rx_mgmt.freq = rx_freq;
  #endif
           ...
           struct nlattr *wmm)
{
  ...
  case NL80211_CMD_AUTHENTICATE:
    #ifdef ETHANOL
    mlme_event_auth(drv, nla_data(frame), nla_len(frame), freq, sig);
    #else
  ...
  case NL80211_CMD_ASSOCIATE:
    #ifdef ETHANOL
    mlme_event_assoc(drv, nla_data(frame), nla_len(frame), wmm, freq, sig);
    #else
  ...
  case NL80211_CMD_DEAUTHENTICATE:
    #ifdef ETHANOL
    mlme_event_deauth_disassoc(drv, EVENT_DEAUTH,nla_data(frame), nla_len(frame), freq, sig);
    #else
  ...
  case NL80211_CMD_DISASSOCIATE:
    #ifdef ETHANOL
    mlme_event_deauth_disassoc(drv, EVENT_DISASSOC, nla_data(frame), nla_len(frame), freq, sig);
    #else
